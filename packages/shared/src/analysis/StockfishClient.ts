...
interface WorkerLike { postMessage(message: string): void; terminate(): void; onmessage?: (event: { data: unknown }) => void; }
export class StockfishClient {
  private worker?: WorkerLike; private pending?: Pending; private source: 'stockfish'|'fallback' = 'fallback';
  constructor(workerUrl?: string) { const WorkerCtor = (globalThis as unknown as { Worker?: new (url: string) => WorkerLike }).Worker; if (WorkerCtor && workerUrl) { try { this.worker = new WorkerCtor(workerUrl); this.source='stockfish'; this.worker.onmessage=(event)=>this.receive(String(event.data)); } catch { this.worker=undefined; } } }
